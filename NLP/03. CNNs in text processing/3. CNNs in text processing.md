[Источник](https://lena-voita.github.io/nlp_course/text_classification.html#nn_models_cnn)

![[Screen Shot 2024-01-17 at 15.24.55.png]]

При CNN в NLP возникает проблема переменной размерности слоя после convolution, эта проблема решается при помощи pooling для всего слоя (например, maxpool прямо по всему слою)
<font color="green" size=30>Проверить тому, кто смотрел лекцию (все ок!)</font>

CNN in text preprossesing using, for example, for tasks of finding some patterns through the text (the task of classifying positive/negative sentiment of the text). 
...
![[Screen Shot 2024-01-17 at 16.47.47.png]]