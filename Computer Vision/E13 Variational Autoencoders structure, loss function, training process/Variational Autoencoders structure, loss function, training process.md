...
![[Pasted image 20240113012551.png]]

Если будем брать просто Lrec, то сильно отличается от обычного энкодера не будет, то есть картинка будет такой

![[Pasted image 20240113012626.png]]

Но мы хотим получить более компактные пространства

![[Pasted image 20240113012726.png]]

![[Pasted image 20240113012851.png]]

![[Pasted image 20240113012900.png]]
**The training process of VAE**
The training process of a Variational Autoencoder (VAE) involves optimizing the model's parameters to reconstruct input data accurately and encourage the learned latent space to follow a specific distribution. Here are the key steps in the training process of a VAE:

1. **Objective Function:**
    
    - The VAE is trained using a combination of two objectives: reconstruction loss and regularization term (KL divergence).
2. **Reconstruction Loss:**
    
    - The reconstruction loss measures how well the VAE can reconstruct the input data. It is often chosen based on the type of data, commonly using mean squared error (MSE) for continuous data or binary cross-entropy for binary data.
3. **KL Divergence Term:**
    
    - The regularization term, often expressed as KL divergence, encourages the learned latent space to follow a chosen distribution (usually a standard normal distribution). This helps ensure that the latent space is continuous and can be sampled to generate new data.
4. **Objective Function:**
    
    - The combined objective function is the sum of the reconstruction loss and the KL divergence term. The goal is to minimize this objective, striking a balance between accurate reconstruction and a well-structured latent space.
5. **Stochastic Gradient Descent (SGD):**
    
    - Optimization algorithms, such as stochastic gradient descent, are used to minimize the objective function. Mini-batches of data are sampled, and the gradients with respect to the parameters are computed and used to update the model.
6. **Reparameterization Trick:**
    
    - To make the optimization process smoother, the reparameterization trick is often employed. Instead of directly sampling from the latent space, a sample is generated by combining a deterministic function of the input and a sample from a standard normal distribution.
7. **Backpropagation:**
    
    - Gradients are propagated through the network using backpropagation, allowing the model to adjust its parameters to minimize the objective function.
8. **Iterative Training:**
    
    - The training process is performed iteratively over multiple epochs. Each epoch involves presenting the entire dataset to the model and updating the parameters to improve reconstruction and enforce the desired structure in the latent space.
9. **Validation and Monitoring:**
    
    - The model's performance is monitored on a validation set, and training can be halted based on certain criteria, such as convergence or reaching a satisfactory level of performance.

The key idea is to find a balance between reconstructing the input data accurately and regularizing the latent space to follow a specific distribution. This allows the VAE to learn a meaningful representation of the input data in a continuous and generative manner.
