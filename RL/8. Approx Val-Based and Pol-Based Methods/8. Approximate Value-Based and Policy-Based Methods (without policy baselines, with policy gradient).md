...
### List of methods ###
Approximate value-based and policy-based methods are categories of reinforcement learning algorithms that handle large state spaces by using function approximation techniques, such as neural networks. Here's a list of some commonly used algorithms in each category:

## Approximate Value-Based Methods:

1. **Deep Q-Learning (DQN):**
   - DQN combines Q-learning with deep neural networks to approximate the action-value function. It introduces experience replay and target networks to stabilize learning.

2. **Double Deep Q-Learning (DDQN):**
   - DDQN is an extension of DQN that addresses overestimation bias in action values by using two separate networks to select and evaluate actions.

3. **Deep Deterministic Policy Gradient (DDPG):**
   - DDPG is an algorithm for continuous action spaces that combines deep Q-learning with policy gradients. It maintains an actor and a critic network.

4. **Twin Delayed DDPG (TD3):**
   - TD3 is an improvement over DDPG that addresses issues with overestimation bias. It introduces a pair of critics and uses the minimum value among them.

5. **Advantage Actor-Critic (A2C):**
   - A2C is an actor-critic algorithm that combines advantages (TD errors) with policy gradients. It uses a shared neural network for the actor and critic.

### Approximate Policy-Based Methods (with Policy Gradients):

1. **Vanilla Policy Gradient (REINFORCE):**
   - Vanilla Policy Gradient is a simple policy optimization algorithm that uses the gradient of the expected cumulative reward with respect to the policy parameters.

2. **Actor-Critic:**
   - Actor-Critic methods combine both policy-based and value-based approaches. The actor updates the policy, and the critic evaluates the actions using a value function.

3. **Deep Deterministic Policy Gradient (DDPG):**
   - DDPG, mentioned earlier in the value-based list, can also be considered a policy gradient method due to its actor-critic structure.

4. **Trust Region Policy Optimization (TRPO):**
   - TRPO, mentioned earlier in the value-based list, is a policy optimization algorithm that constrains the policy update using a trust region.

6. **Soft Actor-Critic (SAC):**
   - SAC is an off-policy actor-critic algorithm that incorporates entropy regularisation in the objective function, leading to more exploratory behavior.

7. **Deep Policy Networks (DPN):**
   - DPN is a policy-based method that directly learns a mapping from states to actions using deep neural networks.

8. **Natural Policy Gradient (NPG):**
   - NPG is a policy optimization method that uses the natural gradient, taking into account the curvature of the policy space.
